...
class Node(object):
    ...
    def show(self, buf=sys.stdout, offset=0, attrnames=False, nodenames=False, showcoord=False, _my_node_name=None): #buf=sys.stdout
    ...
    def write(self, buf=None, offset=0, attrnames=False, nodenames=False, showcoord=False, _my_node_name=None): #
        if buf == None:
            buf = []
        ...
        lead = ' ' * offset
        if nodenames and _my_node_name is not None:
            string += self.__class__.__name__+ ' <' + _my_node_name + '>: '
        else:
